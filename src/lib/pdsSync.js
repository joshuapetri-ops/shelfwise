...
function recordToBook(record, uri) {
  return {
    ...
    ratings: record.ratings && String(record.ratings).trim().length > 1 ? JSON.parse(record.ratings) : {},
    subjects: record.subjects && String(record.subjects).trim().length > 1 ? JSON.parse(record.subjects) : [],
